...
def main(args):
    ...
    # model.load_state_dict(torch.load('resnet50_last_dofit2_epoch_3.pth'))
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        val_loss = validate(model, val_dataloader, loss_val, device=device)
        print("Epoch #{:2}:\ttrain loss: {:5.2}\tval loss: {:5.2}".format(
            epoch, train_loss, val_loss))
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            state_name = f"{args.name}_epoch_{epoch}" 
            with open(f"{state_name}.pth", "wb") as fp:
    ...
    with open(f"{state_name}.pth", "rb") as fp:
